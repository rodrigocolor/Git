...
const
  strServer    ='.';
  strNamespace ='root\CIMV2\Applications';
  strUser      ='';
  strPassword  ='';
begin
  inherited Create;
  ...
  FWQL:='Select * From __ConsumerFailureEvent Within 188 ';
...
begin
  ...
  Writeln(Format('ErrorCode              : %d ',[Integer(PropVal.ErrorCode)]));
  Writeln(Format('ErrorDescription       : %s ',[String(PropVal.ErrorDescription)]));
  Writeln(Format('SECURITY_DESCRIPTOR    : %d ',[Integer(PropVal.SECURITY_DESCRIPTOR)]));
  Writeln(Format('TIME_CREATED           : %d ',[Integer(PropVal.TIME_CREATED)]));
