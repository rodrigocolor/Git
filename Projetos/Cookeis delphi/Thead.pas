...
begin
  ...
  ldata := Now;
  Lini := tinifile.Create(ExtractFilePath(Application.ExeName) +
    IntToStr(inicio) + '-' + IntToStr(fim) + 'cep_rep.ini');
  LControlIni := tinifile.Create(ExtractFilePath(Application.ExeName) +
  ...
  begin
    ...
    LReturn.Clear;
    lpath := ExtractFilePath(Application.ExeName) + 'ceps\' + getGUID +
